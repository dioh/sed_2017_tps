...
PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock(const string &name) :
    Atomic(name),
    in_port_TotPreySubSubModelStock(addInputPort("in_port_TotPreySubSubModelStock")),
    in_port_subtract(addInputPort("in_port_subtract")),
    in_port_increment(addInputPort("in_port_increment")),
    out_port_PreySubSubModelStock(addOutputPort("out_port_PreySubSubModelStock"))
{
...
#ifdef PREYSUBSUBMODELSTOCKDEVS_BASIC_COUPLED_PREYSUBSUBMODELSTOCK_LOG_OUTPUT
    log_output_PreySubSubModelStock = true;
#else
    log_output_PreySubSubModelStock = false;
#endif

    if(log_output_PreySubSubModelStock)
    {
...
VTime PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::initFunction()
{
    sigma = VTime::Zero;
...
Model &PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotPreySubSubModelStock)
    {
...
Model &PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::internalFunction(const InternalMessage &msg)
{
...
Model &PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_PreySubSubModelStock, out_value);

    return *this ;
}

double PreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::get_param(const string &name)
{
    double value = 0;
