...
TotPreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::TotPreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock(const string &name) :
	Atomic(name),
	in_minus_port_Flow1_PreySubSubModelStock(addInputPort("in_minus_port_Flow1_PreySubSubModelStock")),
...
Model &TotPreySubSubModelStockDEVS_BASIC_COUPLED_PreySubSubModelStock::externalFunction(const ExternalMessage &msg)
{
	...
	if(msg.port() == in_minus_port_Flow1_PreySubSubModelStock) {
...
{
	double plus = 0;
	double minus = 0;if(isSet_Flow1_PreySubSubModelStock) { minus = minus + Flow1_PreySubSubModelStock; }double val = plus - minus;
