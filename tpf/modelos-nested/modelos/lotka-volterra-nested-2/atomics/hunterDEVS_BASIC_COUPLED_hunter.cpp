...
hunterDEVS_BASIC_COUPLED_hunter::hunterDEVS_BASIC_COUPLED_hunter(const string &name) :
    Atomic(name),
    in_port_Tothunter(addInputPort("in_port_Tothunter")),
    in_port_subtract(addInputPort("in_port_subtract")),
    in_port_increment(addInputPort("in_port_increment")),
    out_port_hunter(addOutputPort("out_port_hunter"))
{
...
#ifdef HUNTERDEVS_BASIC_COUPLED_HUNTER_LOG_OUTPUT
    log_output_hunter = true;
#else
    log_output_hunter = false;
#endif

    if(log_output_hunter)
    {
...
VTime hunterDEVS_BASIC_COUPLED_hunter::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double hunterDEVS_BASIC_COUPLED_hunter::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &hunterDEVS_BASIC_COUPLED_hunter::initFunction()
{
    sigma = VTime::Zero;
...
Model &hunterDEVS_BASIC_COUPLED_hunter::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &hunterDEVS_BASIC_COUPLED_hunter::internalFunction(const InternalMessage &msg)
{
...
Model &hunterDEVS_BASIC_COUPLED_hunter::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_hunter, out_value);

    return *this ;
}

double hunterDEVS_BASIC_COUPLED_hunter::get_param(const string &name)
{
    double value = 0;
