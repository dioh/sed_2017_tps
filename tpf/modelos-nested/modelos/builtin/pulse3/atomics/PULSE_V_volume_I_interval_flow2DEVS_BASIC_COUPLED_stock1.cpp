...
PULSE_V_volume_I_interval_flow2DEVS_BASIC_COUPLED_stock1::PULSE_V_volume_I_interval_flow2DEVS_BASIC_COUPLED_stock1(const string &name) :
	Atomic(name),
	...
	dist(0,100),
	rng(random_device()()),
	out_port_PULSE_V_volume_I_interval_flow2(addOutputPort("out_port_PULSE_V_volume_I_interval_flow2"))
{
}


Model &PULSE_V_volume_I_interval_flow2DEVS_BASIC_COUPLED_stock1::initFunction()
{
...
Model &PULSE_V_volume_I_interval_flow2DEVS_BASIC_COUPLED_stock1::externalFunction(const ExternalMessage &msg)
{
...
Model &PULSE_V_volume_I_interval_flow2DEVS_BASIC_COUPLED_stock1::internalFunction(const InternalMessage &)
{
	if(this->on)
		holdIn(AtomicState::active, this->frequency_time);
	else
...
Model &PULSE_V_volume_I_interval_flow2DEVS_BASIC_COUPLED_stock1::outputFunction(const CollectMessage &msg)
{
	auto random_int = this->volume;//this->dist(this->rng);
	Tuple<Real> out_value{Real(random_int)};
	sendOutput(msg.time(), out_port_PULSE_V_volume_I_interval_flow2, out_value);
	return *this ;
}