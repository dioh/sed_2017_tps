...
WorkersNW::WorkersNW(const string &name) :
    Atomic(name),
...
VTime WorkersNW::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double WorkersNW::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &WorkersNW::initFunction()
{
    sigma = VTime::Zero;
...
Model &WorkersNW::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &WorkersNW::internalFunction(const InternalMessage &msg)
{
...
Model &WorkersNW::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double WorkersNW::get_param(const string &name)
{
    double value = 0;
