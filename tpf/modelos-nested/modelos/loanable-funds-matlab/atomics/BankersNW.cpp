...
BankersNW::BankersNW(const string &name) :
    Atomic(name),
...
VTime BankersNW::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double BankersNW::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &BankersNW::initFunction()
{
    sigma = VTime::Zero;
...
Model &BankersNW::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &BankersNW::internalFunction(const InternalMessage &msg)
{
...
Model &BankersNW::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double BankersNW::get_param(const string &name)
{
    double value = 0;
