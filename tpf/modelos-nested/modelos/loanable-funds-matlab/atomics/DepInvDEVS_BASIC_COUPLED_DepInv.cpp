...
DepInvDEVS_BASIC_COUPLED_DepInv::DepInvDEVS_BASIC_COUPLED_DepInv(const string &name) :
    Atomic(name),
...
#ifdef DEPINVDEVS_BASIC_COUPLED_DEPINV_LOG_OUTPUT
...
VTime DepInvDEVS_BASIC_COUPLED_DepInv::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double DepInvDEVS_BASIC_COUPLED_DepInv::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &DepInvDEVS_BASIC_COUPLED_DepInv::initFunction()
{
    sigma = VTime::Zero;
...
Model &DepInvDEVS_BASIC_COUPLED_DepInv::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &DepInvDEVS_BASIC_COUPLED_DepInv::internalFunction(const InternalMessage &msg)
{
...
Model &DepInvDEVS_BASIC_COUPLED_DepInv::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double DepInvDEVS_BASIC_COUPLED_DepInv::get_param(const string &name)
{
    double value = 0;
