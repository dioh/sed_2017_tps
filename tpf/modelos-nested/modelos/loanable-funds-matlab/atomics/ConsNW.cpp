...
ConsNW::ConsNW(const string &name) :
    Atomic(name),
...
VTime ConsNW::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double ConsNW::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &ConsNW::initFunction()
{
    sigma = VTime::Zero;
...
Model &ConsNW::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &ConsNW::internalFunction(const InternalMessage &msg)
{
...
Model &ConsNW::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double ConsNW::get_param(const string &name)
{
    double value = 0;
