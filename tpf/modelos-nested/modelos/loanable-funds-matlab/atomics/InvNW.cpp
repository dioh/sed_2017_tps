...
VTime InvNW::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double InvNW::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &InvNW::initFunction()
{
    sigma = VTime::Zero;
...
Model &InvNW::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &InvNW::internalFunction(const InternalMessage &msg)
{
...
Model &InvNW::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double InvNW::get_param(const string &name)
{
    double value = 0;
