...
DepCons::DepCons(const string &name) :
    Atomic(name),
...
VTime DepCons::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double DepCons::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &DepCons::initFunction()
{
    sigma = VTime::Zero;
...
Model &DepCons::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &DepCons::internalFunction(const InternalMessage &msg)
{
...
Model &DepCons::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double DepCons::get_param(const string &name)
{
    double value = 0;
