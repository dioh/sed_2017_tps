...
BankersNWDEVS_BASIC_COUPLED_BankersNW::BankersNWDEVS_BASIC_COUPLED_BankersNW(const string &name) :
    Atomic(name),
    in_port_TotBankersNW(addInputPort("in_port_TotBankersNW")),
    out_port_BankersNW(addOutputPort("out_port_BankersNW"))
{
...
#ifdef BANKERSNWDEVS_BASIC_COUPLED_BANKERSNW_LOG_OUTPUT
    log_output_BankersNW = true;
#else
    log_output_BankersNW = false;
#endif

    if(log_output_BankersNW)
    {
...
VTime BankersNWDEVS_BASIC_COUPLED_BankersNW::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double BankersNWDEVS_BASIC_COUPLED_BankersNW::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &BankersNWDEVS_BASIC_COUPLED_BankersNW::initFunction()
{
    sigma = VTime::Zero;
...
Model &BankersNWDEVS_BASIC_COUPLED_BankersNW::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotBankersNW)
    {
...
Model &BankersNWDEVS_BASIC_COUPLED_BankersNW::internalFunction(const InternalMessage &msg)
{
...
Model &BankersNWDEVS_BASIC_COUPLED_BankersNW::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_BankersNW, out_value);

    return *this ;
}

double BankersNWDEVS_BASIC_COUPLED_BankersNW::get_param(const string &name)
{
    double value = 0;
