...
Workers::Workers(const string &name) :
    Atomic(name),
...
VTime Workers::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double Workers::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &Workers::initFunction()
{
    sigma = VTime::Zero;
...
Model &Workers::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &Workers::internalFunction(const InternalMessage &msg)
{
...
Model &Workers::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double Workers::get_param(const string &name)
{
    double value = 0;
