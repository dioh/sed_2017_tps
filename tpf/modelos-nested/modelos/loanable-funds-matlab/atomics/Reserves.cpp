...
Reserves::Reserves(const string &name) :
    Atomic(name),
...
VTime Reserves::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double Reserves::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &Reserves::initFunction()
{
    sigma = VTime::Zero;
...
Model &Reserves::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &Reserves::internalFunction(const InternalMessage &msg)
{
...
Model &Reserves::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double Reserves::get_param(const string &name)
{
    double value = 0;
