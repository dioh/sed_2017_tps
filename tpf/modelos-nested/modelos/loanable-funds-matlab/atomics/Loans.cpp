...
VTime Loans::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double Loans::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &Loans::initFunction()
{
    sigma = VTime::Zero;
...
Model &Loans::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &Loans::internalFunction(const InternalMessage &msg)
{
...
Model &Loans::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double Loans::get_param(const string &name)
{
    double value = 0;
