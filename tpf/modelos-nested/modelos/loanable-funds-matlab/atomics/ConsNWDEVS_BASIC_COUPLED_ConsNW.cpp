...
ConsNWDEVS_BASIC_COUPLED_ConsNW::ConsNWDEVS_BASIC_COUPLED_ConsNW(const string &name) :
    Atomic(name),
    in_port_TotConsNW(addInputPort("in_port_TotConsNW")),
    out_port_ConsNW(addOutputPort("out_port_ConsNW"))
{
...
#ifdef CONSNWDEVS_BASIC_COUPLED_CONSNW_LOG_OUTPUT
    log_output_ConsNW = true;
#else
    log_output_ConsNW = false;
#endif

    if(log_output_ConsNW)
    {
...
VTime ConsNWDEVS_BASIC_COUPLED_ConsNW::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double ConsNWDEVS_BASIC_COUPLED_ConsNW::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &ConsNWDEVS_BASIC_COUPLED_ConsNW::initFunction()
{
    sigma = VTime::Zero;
...
Model &ConsNWDEVS_BASIC_COUPLED_ConsNW::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &ConsNWDEVS_BASIC_COUPLED_ConsNW::internalFunction(const InternalMessage &msg)
{
...
Model &ConsNWDEVS_BASIC_COUPLED_ConsNW::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_ConsNW, out_value);

    return *this ;
}

double ConsNWDEVS_BASIC_COUPLED_ConsNW::get_param(const string &name)
{
    double value = 0;
