...
ReservesDEVS_BASIC_COUPLED_Reserves::ReservesDEVS_BASIC_COUPLED_Reserves(const string &name) :
    Atomic(name),
    in_port_TotReserves(addInputPort("in_port_TotReserves")),
    out_port_Reserves(addOutputPort("out_port_Reserves"))
{
...
#ifdef RESERVESDEVS_BASIC_COUPLED_RESERVES_LOG_OUTPUT
    log_output_Reserves = true;
#else
    log_output_Reserves = false;
#endif

    if(log_output_Reserves)
    {
...
VTime ReservesDEVS_BASIC_COUPLED_Reserves::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double ReservesDEVS_BASIC_COUPLED_Reserves::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &ReservesDEVS_BASIC_COUPLED_Reserves::initFunction()
{
    sigma = VTime::Zero;
...
Model &ReservesDEVS_BASIC_COUPLED_Reserves::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotReserves)
    {
...
Model &ReservesDEVS_BASIC_COUPLED_Reserves::internalFunction(const InternalMessage &msg)
{
...
Model &ReservesDEVS_BASIC_COUPLED_Reserves::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_Reserves, out_value);

    return *this ;
}

double ReservesDEVS_BASIC_COUPLED_Reserves::get_param(const string &name)
{
    double value = 0;
