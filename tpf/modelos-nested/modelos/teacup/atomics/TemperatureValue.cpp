...
TemperatureValue::TemperatureValue(const string &name) :
    Atomic(name),
...
#ifdef TEMPERATUREVALUE_LOG_OUTPUT
...
VTime TemperatureValue::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double TemperatureValue::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &TemperatureValue::initFunction()
{
    sigma = VTime::Zero;
...
Model &TemperatureValue::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &TemperatureValue::internalFunction(const InternalMessage &msg)
{
...
Model &TemperatureValue::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double TemperatureValue::get_param(const string &name)
{
    double value = 0;
