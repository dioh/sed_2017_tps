...
CapitalDEVS_BASIC_COUPLED_Capital::CapitalDEVS_BASIC_COUPLED_Capital(const string &name) :
    Atomic(name),
    in_port_TotCapital(addInputPort("in_port_TotCapital")),
    out_port_Capital(addOutputPort("out_port_Capital"))
{
...
#ifdef CAPITALDEVS_BASIC_COUPLED_CAPITAL_LOG_OUTPUT
    log_output_Capital = true;
#else
    log_output_Capital = false;
#endif

    if(log_output_Capital)
    {
...
VTime CapitalDEVS_BASIC_COUPLED_Capital::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double CapitalDEVS_BASIC_COUPLED_Capital::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &CapitalDEVS_BASIC_COUPLED_Capital::initFunction()
{
    sigma = VTime::Zero;
...
Model &CapitalDEVS_BASIC_COUPLED_Capital::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotCapital)
    {
...
Model &CapitalDEVS_BASIC_COUPLED_Capital::internalFunction(const InternalMessage &msg)
{
...
Model &CapitalDEVS_BASIC_COUPLED_Capital::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_Capital, out_value);

    return *this ;
}

double CapitalDEVS_BASIC_COUPLED_Capital::get_param(const string &name)
{
    double value = 0;
