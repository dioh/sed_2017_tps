...
wageRateDEVS_BASIC_COUPLED_wageRate::wageRateDEVS_BASIC_COUPLED_wageRate(const string &name) :
    Atomic(name),
    in_port_TotwageRate(addInputPort("in_port_TotwageRate")),
    in_port_subtract(addInputPort("in_port_subtract")),
    in_port_increment(addInputPort("in_port_increment")),
    out_port_wageRate(addOutputPort("out_port_wageRate"))
{
...
#ifdef WAGERATEDEVS_BASIC_COUPLED_WAGERATE_LOG_OUTPUT
    log_output_wageRate = true;
#else
    log_output_wageRate = false;
#endif

    if(log_output_wageRate)
    {
...
VTime wageRateDEVS_BASIC_COUPLED_wageRate::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double wageRateDEVS_BASIC_COUPLED_wageRate::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &wageRateDEVS_BASIC_COUPLED_wageRate::initFunction()
{
    sigma = VTime::Zero;
...
Model &wageRateDEVS_BASIC_COUPLED_wageRate::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotwageRate)
    {
...
Model &wageRateDEVS_BASIC_COUPLED_wageRate::internalFunction(const InternalMessage &msg)
{
...
Model &wageRateDEVS_BASIC_COUPLED_wageRate::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_wageRate, out_value);

    return *this ;
}

double wageRateDEVS_BASIC_COUPLED_wageRate::get_param(const string &name)
{
    double value = 0;
