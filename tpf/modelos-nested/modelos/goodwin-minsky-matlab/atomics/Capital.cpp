...
Capital::Capital(const string &name) :
    Atomic(name),
...
VTime Capital::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double Capital::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &Capital::initFunction()
{
    sigma = VTime::Zero;
...
Model &Capital::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &Capital::internalFunction(const InternalMessage &msg)
{
...
Model &Capital::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double Capital::get_param(const string &name)
{
    double value = 0;
