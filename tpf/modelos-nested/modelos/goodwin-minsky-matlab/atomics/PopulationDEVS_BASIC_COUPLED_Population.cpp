...
#ifdef POPULATIONDEVS_BASIC_COUPLED_POPULATION_LOG_OUTPUT
    log_output_Population = true;
#else
    log_output_Population = false;
#endif

    if(log_output_Population)
    {
...
VTime PopulationDEVS_BASIC_COUPLED_Population::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double PopulationDEVS_BASIC_COUPLED_Population::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &PopulationDEVS_BASIC_COUPLED_Population::initFunction()
{
    sigma = VTime::Zero;
...
Model &PopulationDEVS_BASIC_COUPLED_Population::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotPopulation)
    {
...
Model &PopulationDEVS_BASIC_COUPLED_Population::internalFunction(const InternalMessage &msg)
{
...
Model &PopulationDEVS_BASIC_COUPLED_Population::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_Population, out_value);

    return *this ;
}

double PopulationDEVS_BASIC_COUPLED_Population::get_param(const string &name)
{
    double value = 0;
