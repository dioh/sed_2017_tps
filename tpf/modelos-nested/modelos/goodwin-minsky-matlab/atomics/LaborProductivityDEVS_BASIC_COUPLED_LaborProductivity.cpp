...
#ifdef LABORPRODUCTIVITYDEVS_BASIC_COUPLED_LABORPRODUCTIVITY_LOG_OUTPUT
    log_output_LaborProductivity = true;
#else
    log_output_LaborProductivity = false;
#endif

    if(log_output_LaborProductivity)
    {
...
VTime LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::initFunction()
{
    sigma = VTime::Zero;
...
Model &LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
    ...
    if(msg.port() == in_port_TotLaborProductivity)
    {
...
Model &LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::internalFunction(const InternalMessage &msg)
{
...
Model &LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
    ...
    sendOutput(msg.time(), out_port_LaborProductivity, out_value);

    return *this ;
}

double LaborProductivityDEVS_BASIC_COUPLED_LaborProductivity::get_param(const string &name)
{
    double value = 0;
