...
#include "LaborProductivity.h"

using namespace std;


LaborProductivity::LaborProductivity(const string &name) :
    Atomic(name),
...
#ifdef LABORPRODUCTIVITY_LOG_OUTPUT
...
VTime LaborProductivity::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double LaborProductivity::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &LaborProductivity::initFunction()
{
    sigma = VTime::Zero;
...
Model &LaborProductivity::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &LaborProductivity::internalFunction(const InternalMessage &msg)
{
...
Model &LaborProductivity::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double LaborProductivity::get_param(const string &name)
{
    double value = 0;
