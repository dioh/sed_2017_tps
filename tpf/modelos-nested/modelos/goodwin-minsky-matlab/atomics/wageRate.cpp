...
wageRate::wageRate(const string &name) :
    Atomic(name),
...
VTime wageRate::minposroot(double *coeff)
{
    float mpr;
    VTime ret;
...
double wageRate::to_seconds(const VTime &vt)
{
    return vt.asMsecs() / 1000.;
}


Model &wageRate::initFunction()
{
    sigma = VTime::Zero;
...
Model &wageRate::externalFunction(const ExternalMessage &msg)
{
    double diffxq[2];
...
Model &wageRate::internalFunction(const InternalMessage &msg)
{
...
Model &wageRate::outputFunction(const CollectMessage &msg)
{
    double y[2] = {x[0], x[1]};
...
double wageRate::get_param(const string &name)
{
    double value = 0;
