...
def preprocessing_devsml_for_ma(root):
	...
	def traverse(r, level):
		...
		print atomics_names
		for c in coupleds:
			internal_connections = c.find('./internal_connections')
			ext_input_conn  = c.find('./external_input_connections')
			ext_output_conn = c.find('./external_output_connections')
			...
			for eoc in ext_output_conn:
				if eoc.get('component_from') in atomics_names:
					ext_output_conn.set('component_from', eoc.get('component_from') + str(level))
